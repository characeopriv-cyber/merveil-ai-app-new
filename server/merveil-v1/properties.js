import { json, requireApiKey, logApiUsage, supabaseAdmin } from './_lib.js';

export default async function handler(req, res) {
  if (req.method === 'OPTIONS') return json(res, 204, null);
  const auth = await requireApiKey(req, 'properties:read', res);
  if (auth.error) return json(res, auth.status || 401, { error: auth.status === 403 ? 'insufficient_scope' : 'unauthorized', message: auth.error, request_id: auth.requestId }, auth.headers);
  if (req.method !== 'GET') { await logApiUsage(auth, req, 405); return json(res, 405, { error: 'method_not_allowed', request_id: auth.requestId }); }
  const limit = Math.min(Math.max(Number(req.query?.limit || 20), 1), 100);
  let query = supabaseAdmin.from('properties').select('id,title,area,emirate,price,beds,baths,sqft,furnished,description,photo_url,photo_urls,video_url,listing_type,category,price_frequency,country,created_at').order('created_at', { ascending: false }).limit(limit);
  if (req.query?.country) query = query.eq('country', String(req.query.country).slice(0, 80));
  if (req.query?.emirate) query = query.eq('emirate', String(req.query.emirate).slice(0, 80));
  if (req.query?.category) query = query.eq('category', String(req.query.category).slice(0, 80));
  const { data, error } = await query;
  if (error) { await logApiUsage(auth, req, 500); return json(res, 500, { error: 'database_error', request_id: auth.requestId }); }
  await logApiUsage(auth, req, 200);
  return json(res, 200, { data: data || [], request_id: auth.requestId });
}
