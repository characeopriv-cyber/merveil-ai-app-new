...
export default async function handler(req,res){
  ...
  const auth=await requireApiKey(req,'verification:read',res);
  ...
  const {data,error}=await supabaseAdmin.from('verifications').select('id,type,level,status,reviewed_at,created_at,updated_at').eq('user_id',auth.app.user_id).order('created_at',{ascending:false}).limit(20);
